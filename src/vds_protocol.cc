...
constexpr std::size_t kOutputRumbleRightOffset =
    offsetof(vds_set_state_data, rumble_emulation_right);
constexpr std::size_t kOutputRumbleLeftOffset =
    offsetof(vds_set_state_data, rumble_emulation_left);
constexpr std::uint8_t kOutputRumbleCompatibilityEnable = 0x03;
...
constexpr std::uint8_t kOutputFlag0RightTriggerFfbEnable = 0x04;
constexpr std::uint8_t kOutputFlag0LeftTriggerFfbEnable = 0x08;
constexpr std::uint8_t kOutputFlag1LedColorEnable = 0x04;
constexpr std::uint8_t kOutputFlag1PlayerIndicatorsEnable = 0x10;
constexpr std::size_t kOutputRightTriggerFfbOffset =
    offsetof(vds_set_state_data, right_trigger_ffb);
constexpr std::size_t kOutputLeftTriggerFfbOffset =
    offsetof(vds_set_state_data, left_trigger_ffb);
constexpr std::size_t kOutputPlayerLightsOffset =
    offsetof(vds_set_state_data, led_red) - 1;

unsigned parse_effect_number(std::string_view text, std::string_view spec) {
  unsigned value = 0;
  bool any = false;
  for (const char character : text) {
    if (character < '0' || character > '9') {
      throw std::invalid_argument("invalid trigger effect number in spec: " +
                                  std::string(spec));
    }
  ...
  if (!any) {
    throw std::invalid_argument("empty trigger effect number in spec: " +
                                std::string(spec));
  }
  return value;
}

std::vector<unsigned> parse_effect_params(std::string_view params,
                                          std::string_view spec) {
  std::vector<unsigned> values;
  while (!params.empty()) {
    const std::size_t comma = params.find(',');
    values.push_back(
        parse_effect_number(params.substr(0, comma), spec));
...
void require_effect_range(bool valid, const char *what) {
  if (!valid) {
    throw std::invalid_argument(std::string("trigger effect parameter out of "
                                            "range: ") +
                                what);
  }
}
...
TriggerFfb trigger_effect_feedback(unsigned position, unsigned strength) {
  require_effect_range(position <= 9, "feedback position 0-9");
  require_effect_range(strength >= 1 && strength <= 8, "feedback strength 1-8");
  const std::uint8_t force = static_cast<std::uint8_t>((strength - 1) & 0x07);
  std::uint32_t force_zones = 0;
  std::uint16_t active_zones = 0;
  for (unsigned zone = position; zone < 10; ++zone) {
    force_zones |= static_cast<std::uint32_t>(force) << (3 * zone);
    active_zones |= static_cast<std::uint16_t>(1u << zone);
  }
  TriggerFfb effect{};
  effect[0] = 0x21;
  effect[1] = static_cast<std::uint8_t>(active_zones & 0xff);
  effect[2] = static_cast<std::uint8_t>((active_zones >> 8) & 0xff);
  effect[3] = static_cast<std::uint8_t>(force_zones & 0xff);
  effect[4] = static_cast<std::uint8_t>((force_zones >> 8) & 0xff);
  effect[5] = static_cast<std::uint8_t>((force_zones >> 16) & 0xff);
  effect[6] = static_cast<std::uint8_t>((force_zones >> 24) & 0xff);
  return effect;
}

TriggerFfb trigger_effect_weapon(unsigned start, unsigned end,
                                 unsigned strength) {
  require_effect_range(start >= 2 && start <= 7, "weapon start 2-7");
  require_effect_range(end > start && end <= 8, "weapon end start+1-8");
  require_effect_range(strength >= 1 && strength <= 8, "weapon strength 1-8");
  const std::uint16_t zones =
      static_cast<std::uint16_t>((1u << start) | (1u << end));
  TriggerFfb effect{};
  effect[0] = 0x25;
  effect[1] = static_cast<std::uint8_t>(zones & 0xff);
  effect[2] = static_cast<std::uint8_t>((zones >> 8) & 0xff);
  effect[3] = static_cast<std::uint8_t>(strength - 1);
  return effect;
}

TriggerFfb trigger_effect_vibration(unsigned position, unsigned amplitude,
                                    unsigned frequency) {
  require_effect_range(position <= 9, "vibration position 0-9");
  require_effect_range(amplitude >= 1 && amplitude <= 8,
                       "vibration amplitude 1-8");
  require_effect_range(frequency >= 1 && frequency <= 255,
  ...
  std::uint16_t active_zones = 0;
  for (unsigned zone = position; zone < 10; ++zone) {
    amplitude_zones |= static_cast<std::uint32_t>(strength) << (3 * zone);
    active_zones |= static_cast<std::uint16_t>(1u << zone);
  }
  TriggerFfb effect{};
  effect[0] = 0x26;
  effect[1] = static_cast<std::uint8_t>(active_zones & 0xff);
  effect[2] = static_cast<std::uint8_t>((active_zones >> 8) & 0xff);
  effect[3] = static_cast<std::uint8_t>(amplitude_zones & 0xff);
  effect[4] = static_cast<std::uint8_t>((amplitude_zones >> 8) & 0xff);
  effect[5] = static_cast<std::uint8_t>((amplitude_zones >> 16) & 0xff);
  effect[6] = static_cast<std::uint8_t>((amplitude_zones >> 24) & 0xff);
  effect[9] = static_cast<std::uint8_t>(frequency);
  return effect;
}

TriggerFfb trigger_effect_bow(unsigned start, unsigned end, unsigned strength,
                              unsigned snap_force) {
  require_effect_range(start <= 8, "bow start 0-8");
  require_effect_range(end > start && end <= 8, "bow end start+1-8");
  require_effect_range(strength >= 1 && strength <= 8, "bow strength 1-8");
  require_effect_range(snap_force >= 1 && snap_force <= 8,
                       "bow snap force 1-8");
  const std::uint16_t zones =
      static_cast<std::uint16_t>((1u << start) | (1u << end));
  const std::uint32_t force_pair =
      static_cast<std::uint32_t>((strength - 1) & 0x07) |
      (static_cast<std::uint32_t>((snap_force - 1) & 0x07) << 3);
  TriggerFfb effect{};
  effect[0] = 0x22;
  effect[1] = static_cast<std::uint8_t>(zones & 0xff);
  effect[2] = static_cast<std::uint8_t>((zones >> 8) & 0xff);
  effect[3] = static_cast<std::uint8_t>(force_pair & 0xff);
  effect[4] = static_cast<std::uint8_t>((force_pair >> 8) & 0xff);
  return effect;
}
...
TriggerFfb trigger_effect_galloping(unsigned start, unsigned end,
                                    ...
                                    unsigned frequency) {
  require_effect_range(start <= 8, "galloping start 0-8");
  require_effect_range(end > start && end <= 9, "galloping end start+1-9");
  require_effect_range(first_foot <= 6, "galloping first foot 0-6");
  require_effect_range(second_foot > first_foot && second_foot <= 7,
                       "galloping second foot first+1-7");
  require_effect_range(frequency >= 1 && frequency <= 255,
                       "galloping frequency 1-255");
  const std::uint16_t zones =
      static_cast<std::uint16_t>((1u << start) | (1u << end));
  const std::uint8_t ratio = static_cast<std::uint8_t>(
      (second_foot & 0x07) | ((first_foot & 0x07) << 3));
  TriggerFfb effect{};
  effect[0] = 0x23;
  effect[1] = static_cast<std::uint8_t>(zones & 0xff);
  effect[2] = static_cast<std::uint8_t>((zones >> 8) & 0xff);
  effect[3] = ratio;
  effect[4] = static_cast<std::uint8_t>(frequency);
  return effect;
}
...
TriggerFfb trigger_effect_machine(unsigned start, unsigned end,
  ...
  require_effect_range(start <= 8, "machine start 0-8");
  require_effect_range(end > start && end <= 9, "machine end start+1-9");
  require_effect_range(strength_a <= 7, "machine strength A 0-7");
  require_effect_range(strength_b <= 7, "machine strength B 0-7");
  require_effect_range(frequency >= 1 && frequency <= 255,
                       "machine frequency 1-255");
  require_effect_range(period <= 255, "machine period 0-255");
  const std::uint16_t zones =
      static_cast<std::uint16_t>((1u << start) | (1u << end));
  const std::uint8_t strength_pair = static_cast<std::uint8_t>(
      (strength_a & 0x07) | ((strength_b & 0x07) << 3));
  TriggerFfb effect{};
  effect[0] = 0x27;
  effect[1] = static_cast<std::uint8_t>(zones & 0xff);
  effect[2] = static_cast<std::uint8_t>((zones >> 8) & 0xff);
  effect[3] = strength_pair;
  effect[4] = static_cast<std::uint8_t>(frequency);
  effect[5] = static_cast<std::uint8_t>(period);
  return effect;
}
...
TriggerFfb parse_trigger_effect_spec(std::string_view spec) {
  ...
  if (name == "raw") {
    if (params_text.size() != kTriggerFfbSize * 2) {
      throw std::invalid_argument(
          "raw trigger effect must be exactly 22 hex digits");
    }
    TriggerFfb effect{};
    for (std::size_t index = 0; index < kTriggerFfbSize; ++index) {
      const auto nibble = [&](char character) -> std::uint8_t {
        ...
        throw std::invalid_argument("invalid hex digit in raw trigger effect");
      };
      effect[index] = static_cast<std::uint8_t>(
          (nibble(params_text[index * 2]) << 4) |
          nibble(params_text[index * 2 + 1]));
    }
    return effect;
  }

  const std::vector<unsigned> params = parse_effect_params(params_text, spec);
  const auto require_count = [&](std::size_t count) {
    if (params.size() != count) {
      throw std::invalid_argument("trigger effect '" + std::string(name) +
                                  "' expects " + std::to_string(count) +
                                  " parameters");
    }
  };
  if (name == "feedback") {
    require_count(2);
    return trigger_effect_feedback(params[0], params[1]);
  }
  if (name == "weapon") {
    require_count(3);
    return trigger_effect_weapon(params[0], params[1], params[2]);
  }
  if (name == "vibration") {
    require_count(3);
    return trigger_effect_vibration(params[0], params[1], params[2]);
  }
  if (name == "bow") {
    require_count(4);
    return trigger_effect_bow(params[0], params[1], params[2], params[3]);
  }
  if (name == "galloping") {
    require_count(5);
    return trigger_effect_galloping(params[0], params[1], params[2], params[3],
                                    params[4]);
  }
  if (name == "machine") {
    require_count(6);
    return trigger_effect_machine(params[0], params[1], params[2], params[3],
                                  params[4], params[5]);
  }
  throw std::invalid_argument("unknown trigger effect: " + std::string(spec));
}

void DsOutputState::apply_authored_effects(const AuthoredEffects &effects) {
  if (effects.right_trigger) {
    state_[kOutputFlag0Offset] |= kOutputFlag0RightTriggerFfbEnable;
    std::copy(effects.right_trigger->begin(), effects.right_trigger->end(),
              state_.begin() + kOutputRightTriggerFfbOffset);
  }
  if (effects.left_trigger) {
    state_[kOutputFlag0Offset] |= kOutputFlag0LeftTriggerFfbEnable;
    std::copy(effects.left_trigger->begin(), effects.left_trigger->end(),
              state_.begin() + kOutputLeftTriggerFfbOffset);
  }
  if (effects.led_color) {
    state_[kOutputFlag1Offset] |= kOutputFlag1LedColorEnable;
    light_color_ = *effects.led_color;
    state_[kOutputLedColorOffset + 0] = (*effects.led_color)[0];
    state_[kOutputLedColorOffset + 1] = (*effects.led_color)[1];
    state_[kOutputLedColorOffset + 2] = (*effects.led_color)[2];
  }
  if (effects.player_lights) {
    state_[kOutputFlag1Offset] |= kOutputFlag1PlayerIndicatorsEnable;
    state_[kOutputPlayerLightsOffset] = *effects.player_lights & 0x3f;
  }
  if (effects.mute_light) {
    state_[kOutputFlag1Offset] |= kOutputFlag1MicMuteLedControlEnable;
    state_[kOutputMuteLedOffset] = *effects.mute_light;
  }
}
...
BtStateReport DsOutputState::build_bt_rumble_report(
    std::uint8_t right_motor, std::uint8_t left_motor) {
  DsState rumble_state = state_;
  rumble_state[kOutputFlag0Offset] |= kOutputRumbleCompatibilityEnable;
  rumble_state[kOutputRumbleRightOffset] = right_motor;
  rumble_state[kOutputRumbleLeftOffset] = left_motor;
  ...
  report_sequence_ = (report_sequence_ + 1) & 0x0f;
  report[2] = 0x10;
  std::copy(rumble_state.begin(), rumble_state.begin() + kSetStateSize,
            report.begin() + kBtStateOffset);
  fill_output_report_checksum(report);
  return report;
}
