...
private:
  ...
  void listener_loop() {
    ...
    while (!stop_requested.load()) {
      ...
      (void)setsockopt(client.get(), IPPROTO_TCP, TCP_NODELAY,
...
void VirtualPort::Impl::pipe_reader_loop() {
  ...
  for (;;) {
    ...
    case VDS_FRAME_USB_HID_IN:
      // HID input reports are snapshots, so queued reports are already stale.
      // Keep only the newest state to avoid latency when Bluetooth produces
      // reports faster than the virtual USB endpoint consumes them.
      hid_in_queue.clear();
