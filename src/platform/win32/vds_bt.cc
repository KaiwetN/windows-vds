...
constexpr std::size_t kMaxQueuedHidAudioOutputWrites = 1;
...
public:
  ...
  void write_interrupt_packet(std::span<const std::uint8_t> packet) override {
    validate_interrupt_packet(packet);
    if (!enqueue_output_report(packet.subspan(1), false)) {
  ...
  try_write_interrupt_packet(std::span<const std::uint8_t> packet) override {
    validate_interrupt_packet(packet);
    return enqueue_output_report(packet.subspan(1), true);
  }
  ...
  std::optional<std::string> take_output_diagnostics(bool force) override {
    ...
    if (output_queued_since_diagnostic_ == 0 &&
        ...
        output_audio_write_errors_since_diagnostic_ == 0 &&
        output_error_.empty() && !output_caps_pending_) {
      return std::nullopt;
    }
    ...
    std::string message =
        ...
        " audio_write_errors=" +
        std::to_string(output_audio_write_error_count_) +
        ...
        " audio_write_error_delta=" +
        std::to_string(output_audio_write_errors_since_diagnostic_) +
        " pending=" + std::to_string(output_queue_.size()) +
        " input_report_length=" + std::to_string(input_report_length_) +
        " output_report_length=" + std::to_string(output_report_length_) +
        " feature_report_length=" + std::to_string(feature_report_length_);
    if (!output_last_audio_error_.empty()) {
      message += " last_audio_error=" + output_last_audio_error_;
    }
...
private:
  ...
  static void
  validate_interrupt_packet(std::span<const std::uint8_t> packet) {
  ...
  Frame read_input_frame() {
    if (!ResetEvent(input_event_.get())) {
      throw std::runtime_error("failed to reset HID read event: " +
                               win32_error_message(GetLastError()));
    }

    OVERLAPPED overlapped{};
    overlapped.hEvent = input_event_.get();
    if (!ReadFile(handle_.get(), input_report_buffer_.data(),
                  static_cast<DWORD>(input_report_buffer_.size()), nullptr,
                  &overlapped)) {
      const DWORD error = GetLastError();
      if (error != ERROR_IO_PENDING) {
    ...
    for (;;) {
      HANDLE wait_handles[3] = {input_event_.get(), queue_event_.get(),
                                stop_event_.get()};
      ...
      if (wait_result == WAIT_OBJECT_0) {
        ...
        Frame frame;
        frame.header.type = VDS_FRAME_BT_INTERRUPT_PACKET;
        frame.header.length = transferred + 1;
        frame.header.sequence = ++sequence_;
        frame.payload.reserve(transferred + 1);
        frame.payload.push_back(kBtHidpInputPrefix);
        frame.payload.insert(frame.payload.end(), input_report_buffer_.begin(),
                             input_report_buffer_.begin() + transferred);
        return frame;
      }
      if (wait_result == WAIT_OBJECT_0 + 1) {
        if (auto frame = pop_queued_frame()) {
  ...
  QueuedOutputWrite make_output_write(std::span<const std::uint8_t> report,
    ...
    std::copy(report.begin(), report.end(), buffer.begin());
    return QueuedOutputWrite{std::move(buffer), is_audio};
  }

  bool enqueue_output_report(std::span<const std::uint8_t> report,
                             bool is_audio_report) {
    QueuedOutputWrite write = make_output_write(report, is_audio_report);
    ...
    for (;;) {
      ...
      try {
      ...
      } catch (const std::exception &error) {
        {
          ...
          output_diagnostic_pending_ = true;
          if (write.is_audio) {
            output_last_audio_error_ = error.what();
            ++output_audio_write_error_count_;
            ++output_audio_write_errors_since_diagnostic_;
          } else {
  ...
  UniqueHandle input_event_;
  UniqueHandle queue_event_;
  UniqueHandle stop_event_;
  std::vector<std::uint8_t> input_report_buffer_;
  ...
  std::string output_last_audio_error_;
  ...
  std::uint64_t output_audio_write_error_count_ = 0;
    ...
    if (CM_Get_Device_IDA(current, id.data(), static_cast<ULONG>(id.size()),
      ...
      if (lower_id.rfind(R"(bthenum\)", 0) == 0) {
        ...
        if (instance_separator != std::string::npos) {
          ...
          const std::size_t address_separator = instance.rfind('&');
          if (address_separator != std::string_view::npos) {
            const std::size_t address_pos = address_separator + 1;
